...
def main():
    ...
    # Obtain directions.
    camera_calibration = record_attachments.camera_calibration
    depth_width = record_tracks.depth_track.width
    depth_height = record_tracks.depth_track.width
    directions = []
    for row in range(depth_height):
        v = row / (depth_height - 1)
        directions_row = []
        for col in range(depth_width):
            u = col / (depth_width - 1)
            direction = camera_calibration.get_direction(glm.vec2(u, v))
            directions_row.append(direction)
        directions.append(directions_row)

    print(f"direction: {directions[1][1]}")
    return
