...
class NativeFrameMapper:
    def __init__(self, from_calibration: CameraCalibration, to_calibration):
        with from_calibration.to_native() as native_from_calibration, to_calibration.to_native() as native_to_calibration:
            self.ptr = lib.rgbd_frame_mapper_ctor(
                native_from_calibration.ptr, native_to_calibration.ptr
            )
    ...
    def map_color_frame(self, color_frame: YuvFrame) -> YuvFrame:
        with color_frame.to_native() as native_color_frame:
            mapped_color_frame_ptr = lib.rgbd_frame_mapper_map_color_frame(
                self.ptr, native_color_frame.ptr
            )
    ...
    def map_depth_frame(self, depth_frame: Int32Frame) -> Int32Frame:
        with depth_frame.to_native() as native_depth_frame:
            mapped_depth_frame_ptr = lib.rgbd_frame_mapper_map_depth_frame(
                self.ptr, native_depth_frame.ptr
            )
