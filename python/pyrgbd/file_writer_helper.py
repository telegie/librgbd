...
class FileWriterHelper:
	...
	def set_calibration(calibration: CameraCalibration):
		self.calibration = calibration

	def set_depth_codec_type(depth_codec_type: DepthCodecType):
		self.depth_codec_type = depth_codec_type

	def set_cover(yuv_frame: YuvFrame):
		self.cover = cover

	def add_color_byte_frame(time_point_us, keyframe, color_bytes):
		self.color_byte_frames.append(VideoByteFrame(time_point_us, keyframe, color_bytes))

	def add_depth_byte_frame(time_point_us, keyframe, depth_bytes):
		self.depth_byte_frames.append(VideoByteFrame(time_point_us, keyframe, depth_bytes))

	def add_audio_frame(audio_frame: FileAudioFrame):
		self.audio_frames.append(audio_frame)

	def add_imu_frame(imu_frame: FileImuFrame):
		self.imu_frames.append(imu_frame)

	def add_trs_frame(trs_frame: FileTrsFrame):
		self.trs_frames.append(imu_frame)

	def write(output_file_path):
		if len(color_byte_frames) != len(depth_byte_frames):
		...
		file_writer.write_cover(cover)

		audio_frame_index = 0
		imu_frame_index = 0
		trs_frame_index = 0
        for video_frame_index in range(len(color_byte_frames)):
        	color_byte_frame = self.color_byte_frames[video_frame_index]
        	depth_byte_frame = self.depth_byte_frames[video_frame_index]
            ...
            while trs_frame_index < len(file.trs_frames):
                trs_frame = file.trs_frames[trs_frame_index]
